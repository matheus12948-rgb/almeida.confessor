...
try {
    ...
    if ($respDesk.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa02-desktop.png", $bytes)
        Write-Output "SUCCESS: Desktop ($($bytes.Length) bytes) saved"
    }

    # 2. Mobile Fullpage Capture (390px)
    [void](Call-CDP "Emulation.setDeviceMetricsOverride" @{
        width = 390
        height = 844
        deviceScaleFactor = 2
    ...
    if ($respMob.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa02-mobile.png", $bytes)
