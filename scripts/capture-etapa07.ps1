...
try {
    ...
    if ($respDesk.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa07-desktop.png", $bytes)
        Write-Output "SUCCESS: Desktop Fullpage ($($bytes.Length) bytes) saved"
    }

    # 2. Desktop Footer-Specific View
    [void](Call-CDP "Runtime.evaluate" @{ expression = "document.getElementById('footer').scrollIntoView({behavior: 'instant', block: 'end'})" })
    ...
    if ($respSec.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa07-footer-desktop.png", $bytesSec)
        Write-Output "SUCCESS: Footer Desktop View ($($bytesSec.Length) bytes) saved"
    }
    ...
    if ($respMob.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa07-mobile.png", $bytes)
        Write-Output "SUCCESS: Mobile Fullpage ($($bytes.Length) bytes) saved"
    }

    # 4. Mobile Footer-Specific View
    [void](Call-CDP "Runtime.evaluate" @{ expression = "document.getElementById('footer').scrollIntoView({behavior: 'instant', block: 'end'})" })
    Start-Sleep -Milliseconds 600
    ...
    if ($respMobSec.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa07-footer-mobile.png", $bytesMobSec)
        Write-Output "SUCCESS: Footer Mobile View ($($bytesMobSec.Length) bytes) saved"
    }
