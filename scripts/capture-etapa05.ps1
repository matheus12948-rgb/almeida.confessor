...
try {
    ...
    if ($respDesk.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa05-desktop.png", $bytes)
        Write-Output "SUCCESS: Desktop ($($bytes.Length) bytes) saved"
    }

    # 2. Desktop Section-Specific View
    [void](Call-CDP "Runtime.evaluate" @{ expression = "document.getElementById('destaque-institucional').scrollIntoView({behavior: 'instant', block: 'center'})" })
    ...
    if ($respSec.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa05-section.png", $bytesSec)
    ...
    if ($respMob.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa05-mobile.png", $bytes)
    ...
    if ($respMobSec.result.data) {
        ...
        [System.IO.File]::WriteAllBytes("$PWD\preview-etapa05-mobile-section.png", $bytesMobSec)
